...
import { AddMedicamentsType, addMedicamentsSchema as formSchema } from "@/components/schemas/pharmacy"
...
export function AddMedicamentsForm({ t }: AddMedicamentsFormProps) {
  ...
  return (
    <Form {...form}>
      ...
      >
        <FormLabel className="text-2xl font-semibold text-gray-800 mb-4 block">{t("title")}</FormLabel>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-grow">
                  <FormControl>
                    <Input {...field} placeholder={t("medicament")} className="w-full" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-6">
          <Button
            type="button"
            variant="secondary"
            onClick={() => append({ id: Date.now(), value: "", number: 1 })}
            className="w-full sm:w-auto transition-all duration-300 ease-in-out hover:bg-gray-200"
          >
            <Plus className="mr-2 h-4 w-4" />
            {t("add")}
          </Button>
          <Button type="submit" className="w-full sm:w-auto transition-all duration-300 ease-in-out hover:bg-blue-600">
            {t("submit")}
          </Button>
        </div>
      </form>
